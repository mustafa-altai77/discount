...
import com.example.discaount.addmedicine.AddMedicineActivity;
import com.example.discaount.local.MedicineAlarm;
import com.example.discaount.utils.ToastMessage;
import com.example.discaount.views.RobotoLightTextView;
...
public class MedicineFragment extends Fragment implements MedicineContract.View, MedicineAdapter.OnItemClickListener {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        FloatingActionButton fab = requireActivity().findViewById(R.id.fab_add_task);
        fab.setImageResource(R.drawable.ic_add_p);
    ...
    public void showLoadingMedicineError() {
        showMessage(getString(R.string.loading_tasks_error),false);
    }
    ...
    public void showSuccessfullySavedMessage() {
        showMessage(getString(R.string.successfully_saved_me_message),true);
    }
    @Override
    public void showMedicineDeletedSuccessfully() {
        showMessage(getString(R.string.successfully_deleted_message),true);
        Calendar calendar = Calendar.getInstance();
        int day = calendar.get(Calendar.DAY_OF_WEEK);
        presenter.onStart(day);
    }

    private void showMessage(String message,boolean i) {
        if (getView() != null)
        {
            ToastMessage toastMessage=new ToastMessage(getActivity());
            if (i==true) {
                toastMessage.messageError(message,false);
            }
            else
            {
                toastMessage.messageError(message,true);
            }
            }}
    ...
    private void showNoTasksViews(String mainText) {
        ...
        noMedIcon.setImageDrawable(getResources().getDrawable(R.drawable.ic_calendar));
